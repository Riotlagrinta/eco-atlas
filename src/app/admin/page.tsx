...
import { Shield, Plus, Upload, Loader2, CheckCircle, AlertCircle, Trash2, Leaf, Film, Camera, Map as MapIcon, Newspaper, Save, Users, Target, Brain, QrCode, BarChart3, Activity, Edit3, MessageSquare, ChevronRight, X } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<'species' | 'obs' | 'users' | 'quizzes' | 'insights'>('species');

  const router = useRouter();
  const supabase = createClient();

  const fetchData = async () => {
    const [s, o, u, q] = await Promise.all([
  ...
  const handleSubmitSpecies = async (e: React.FormEvent) => {
    ...
    if (!error) {
      ...
      setEditingId(null);
      fetchData();
      setStatus({ type: 'success', msg: "Opération réussie !" });
    }
    setLoading(false);
  };
  ...
  const handleSubmitQuiz = async (e: React.FormEvent) => {
    ...
    if (!error) {
      ...
      fetchData();
      setStatus({ type: 'success', msg: "Quizz créé !" });
    }
    setLoading(false);
  };
  ...
  const handleSubmitQuestion = async (e: React.FormEvent) => {
    ...
    if (!error) {
      setQuestionData({ question_text: '', options: ['', '', ''], correct_index: 0, explanation: '' });
      setStatus({ type: 'success', msg: "Question ajoutée au quizz !" });
    }
    setLoading(false);
  };
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      <div className="flex items-center space-x-4 mb-12">
        ...
        <div className="flex-1"><h1 className="text-3xl font-bold text-stone-900">Panel Administrateur</h1><p className="text-stone-500 text-sm">Gestion globale du patrimoine naturel du Togo</p></div>
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        {[
          ...
          <button key={tab.id} onClick={() => {setActiveTab(tab.id as any); setSelectedQuizId(null);}} className={`px-6 py-2.5 rounded-xl font-bold transition-all flex items-center text-xs ${activeTab === tab.id ? 'bg-green-600 text-white shadow-lg shadow-green-600/20' : 'bg-white text-stone-500 border border-stone-200'}`}>
          ...
          {!selectedQuizId ? (
            <>
              <div className="bg-white p-8 rounded-3xl shadow-xl border border-stone-100">
                <h2 className="text-xl font-bold mb-8 flex items-center"><Plus className="mr-2 text-green-600" /> Nouveau Quizz</h2>
                <form onSubmit={handleSubmitQuiz} className="space-y-6">
                  <input type="text" placeholder="Titre" required value={quizData.title} onChange={(e) => setQuizData({...quizData, title: e.target.value})} className="w-full p-4 bg-stone-50 rounded-2xl outline-none" />
                  <textarea placeholder="Description" rows={3} value={quizData.description} onChange={(e) => setQuizData({...quizData, description: e.target.value})} className="w-full p-4 bg-stone-50 rounded-2xl outline-none"></textarea>
                  <button type="submit" className="w-full py-4 bg-stone-900 text-white font-bold rounded-2xl shadow-lg">Créer le quizz</button>
                </form>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {quizList.map(q => (
                  <div key={q.id} className="bg-white p-6 rounded-3xl border border-stone-100 flex items-center justify-between shadow-sm group hover:border-green-500 transition-all cursor-pointer" onClick={() => setSelectedQuizId(q.id)}>
                    <div><h4 className="font-bold text-stone-900">{q.title}</h4><p className="text-xs text-stone-400">Cliquez pour ajouter des questions</p></div>
                    <ChevronRight className="h-5 w-5 text-stone-300 group-hover:text-green-600" />
                  </div>
                ))}
              </div>
            </>
            ...
            <div className="bg-white p-8 rounded-3xl shadow-xl border border-stone-100">
              <button onClick={() => setSelectedQuizId(null)} className="text-stone-400 font-bold text-sm mb-6 flex items-center hover:text-stone-900"><ArrowLeft className="h-4 w-4 mr-2" /> Retour à la liste</button>
              <h2 className="text-xl font-bold mb-8">Ajouter une question au quizz</h2>
              <form onSubmit={handleSubmitQuestion} className="space-y-6">
                <input type="text" placeholder="Texte de la question" required value={questionData.question_text} onChange={(e) => setQuestionData({...questionData, question_text: e.target.value})} className="w-full p-4 bg-stone-50 rounded-2xl outline-none" />
                <div className="grid grid-cols-1 gap-4">
                  {questionData.options.map((opt, i) => (
                    <div key={i} className="flex gap-2 items-center">
                      <input type="text" placeholder={`Option ${i+1}`} required value={opt} onChange={(e) => {
                        const newOpts = [...questionData.options];
                        newOpts[i] = e.target.value;
                        setQuestionData({...questionData, options: newOpts});
                      }} className="flex-1 p-4 bg-stone-50 rounded-2xl outline-none" />
                      <input type="radio" checked={questionData.correct_index === i} onChange={() => setQuestionData({...questionData, correct_index: i})} name="correct" />
                    </div>
                  ))}
                </div>
                <textarea placeholder="Explication (pourquoi c'est la bonne réponse)" rows={2} value={questionData.explanation} onChange={(e) => setQuestionData({...questionData, explanation: e.target.value})} className="w-full p-4 bg-stone-50 rounded-2xl outline-none"></textarea>
                <button type="submit" className="w-full py-4 bg-green-600 text-white font-bold rounded-2xl shadow-lg">Ajouter la question</button>
              </form>
            </div>
          )}
        </div>
            ...
            {speciesList.map(s => (
              <div key={s.id} className="bg-white p-4 rounded-2xl border border-stone-100 flex items-center justify-between shadow-sm">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => handleEditSpecies(s)} className="p-2 text-stone-400 hover:text-green-600 transition-all"><Edit3 className="h-5 w-5" /></button>
                  <button onClick={() => handleDeleteSpecies(s.id)} className="p-2 text-stone-300 hover:text-red-600 transition-all"><Trash2 className="h-5 w-5" /></button>
                </div>
              </div>
            ))}
          </div>
      ...
      {activeTab === 'obs' && (
        <div className="space-y-8">
          <div className="bg-amber-50 p-6 rounded-3xl border border-amber-100 mb-8">
            ...
            <textarea placeholder="Explication pour l'utilisateur..." value={feedback} onChange={(e) => setFeedback(e.target.value)} className="w-full p-4 bg-white border border-amber-200 rounded-2xl outline-none text-sm"></textarea>
          </div>
