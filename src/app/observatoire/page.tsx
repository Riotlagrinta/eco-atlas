...
import { Leaf, Search, AlertTriangle, Loader2, X, Share2, Heart, MessageSquare, Send, User, MapPin, Trees } from 'lucide-react';
...
function ObservatoireContent() {
  ...
  const [activeCategory, setActiveCategory] = useState<'Fauna' | 'Flora'>('Fauna');
  ...
  return (
    <div className="max-w-7xl mx-auto">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-stone-100/90 backdrop-blur-md">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-3xl shadow-2xl relative border border-stone-200">
              ...
              <div className="p-8">
                ...
                <div className="mb-12"><h3 className="text-xl font-bold text-stone-900 mb-6 flex items-center"><MapPin className="h-5 w-5 mr-2 text-green-600" /> Répartition au Togo</h3><div className="h-64 rounded-3xl overflow-hidden border border-stone-200 shadow-inner"><MiniMap filter="species" /></div></div>
                <div className="bg-stone-50 p-8 rounded-3xl">
                  <h3 className="text-xl font-bold text-stone-900 mb-8 flex items-center"><MessageSquare className="h-5 w-5 mr-2 text-green-600" /> Discussion ({comments.length})</h3>
                  <div className="space-y-6 mb-8">
                    {comments.map((c) => (
                      <div key={c.id} className="flex space-x-4">
                        <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center border border-stone-200 overflow-hidden flex-shrink-0">{c.profiles?.avatar_url ? <img src={c.profiles.avatar_url} className="w-full h-full object-cover" /> : <User className="h-5 w-5 text-stone-300" />}</div>
                  ...
                  <form onSubmit={handleSendComment} className="relative"><input type="text" placeholder="Une question ?" value={newComment} onChange={(e) => setNewComment(e.target.value)} className="w-full pl-6 pr-12 py-4 bg-white border border-stone-200 rounded-2xl outline-none focus:ring-2 focus:ring-green-500 shadow-sm" /><button type="submit" className="absolute right-3 top-3 p-2 bg-green-600 text-white rounded-xl hover:bg-green-700 transition-all"><Send className="h-4 w-4" /></button></form>
                </div>
              </div>
            </motion.div>
          </div>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
          <h1 className="text-4xl font-bold text-stone-900 mb-4 flex items-center"><Leaf className="h-10 w-10 text-green-600 mr-4" /> Observatoire</h1>
          <div className="flex p-1 bg-stone-100 rounded-2xl w-fit">
            <button onClick={() => setActiveCategory('Fauna')} className={`px-6 py-2 rounded-xl font-bold text-xs flex items-center transition-all ${activeCategory === 'Fauna' ? 'bg-white text-green-600 shadow-sm' : 'text-stone-400'}`}><User className="h-3 w-3 mr-2" /> Faune</button>
            <button onClick={() => setActiveCategory('Flora')} className={`px-6 py-2 rounded-xl font-bold text-xs flex items-center transition-all ${activeCategory === 'Flora' ? 'bg-white text-green-600 shadow-sm' : 'text-stone-400'}`}><Trees className="h-3 w-3 mr-2" /> Flore</button>
          </div>
        </div>
        <div className="relative w-full md:w-96"><input type="text" placeholder="Rechercher..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-12 pr-4 py-3 bg-stone-50 border border-stone-200 rounded-2xl focus:ring-2 focus:ring-green-500 outline-none transition-all" /><Search className="absolute left-4 top-3.5 h-5 w-5 text-stone-400" /></div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-24 bg-stone-50 rounded-3xl border-2 border-dashed border-stone-200 text-stone-400 font-medium">Aucun résultat dans cette catégorie.</div>
      )}
    </div>
  );
}
